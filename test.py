...
def loadModelAndData(num):
    ...
    return model, val_dials


def decode(num=1):
    model, val_dials = loadModelAndData(num)
    ...
    for ii in range(1):
        ...
        for name, val_file in val_dials.items():
            ...
            output_words, loss_sentence = model.predict(input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor)
        ...
        with open('data/gold_dict.json', 'w') as f:
            json.dump(dict1, f, indent=2)
        with open('data/gen_dict.json', 'w') as f:
